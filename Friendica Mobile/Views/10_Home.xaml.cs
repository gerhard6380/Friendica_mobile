    ...
    public partial class Home : BaseContentPage
    {
        ...
        void ButtonIconButton_Clicked(object sender, System.EventArgs e)
        {
            ...
            if (Device.RuntimePlatform == Device.macOS)
                ScrollViewHomeThreads.ScrollToAsync(0, ScrollViewHomeThreads.Height, true);
            else
                ScrollViewHomeThreads.ScrollToAsync(0, 0, true);
        }

        async void ScrollViewHomeThreads_Scrolled(object sender, Xamarin.Forms.ScrolledEventArgs e)
        {
            ...
            if (atBottom)
            {
                var mvvm = this.BindingContext as HomeViewModel;
                if (mvvm.PostsModel.HomeThreads.Count > 0 && !mvvm.PostsModel.IsLoadingNext)
