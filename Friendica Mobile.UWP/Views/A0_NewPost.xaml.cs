...
namespace Friendica_Mobile.UWP.Views
{
    ...
    public sealed partial class A0_NewPost : Page
    {
        ...
        private async void rebNewPostText_SelectionChanged(object sender, RoutedEventArgs e)
        {
            ...
            if (clicked && !StaticGlobalParameters.NewPostDontSelectRetweetContentAlreadyShown)
            {
                StaticGlobalParameters.NewPostDontSelectRetweetContentAlreadyShown = true;
                var result = await StaticMessageDialog.ShowDialogAsync("", loader.GetString("messageDialogNewPostDontSelectRetweetContent"), loader.GetString("buttonOK"), null, null, 0, 0);
            }
